...
import { addMutualFund, deleteMutualFund, searchMutualFunds, updateMutualFund, syncMutualFundsFromExcel, refreshMFPortfolio } from "@/app/actions/mutual-funds";
...
export default function MutualFundsClient({ initialFunds }: { initialFunds: any[] }) {
    ...
    useEffect(() => {
        const timer = setTimeout(async () => {
            if (formData.symbol.length >= 1 && !isAdding) {
                setIsSearching(true);
                const results = await searchMutualFunds(formData.symbol);
    ...
    const stats = useMemo(() => {
        const totalInvestment = funds.reduce((sum, s) => sum + (s.quantity * s.avgPrice), 0);
        const currentValue = funds.reduce((sum, s) => sum + (s.quantity * s.currentPrice), 0);
    ...
    const handleExcelSync = async (e: React.ChangeEvent<HTMLInputElement>) => {
        ...
        reader.onload = async (event) => {
            ...
            try {
                const result = await syncMutualFundsFromExcel(base64Data);
                if (result.success) {
                    toast.success(`Successfully synced ${result.count} mutual funds from Excel`);
                    window.location.reload();
                }
            } catch (error: any) {
                toast.error(error.message || "Failed to sync mutual funds from Excel");
            } finally {
                setIsSyncing(false);
    ...
    const handleUpdateFund = async (e: React.FormEvent) => {
        ...
        setIsAdding(true);
        try {
            await updateMutualFund(editingFund.id, {
                quantity: parseFloat(formData.quantity),
                avgPrice: parseFloat(formData.avgPrice),
            });
            window.location.reload();
    ...
    return (
        <div className="space-y-4 pb-10">
            <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-2">
                <div>
                    <h1 className="text-xl font-bold tracking-tight">Mutual Fund Portfolio</h1>
                    <p className="text-xs text-muted-foreground">Investment status & real-time NAV performance</p>
                </div>
                ...
                <div className="flex gap-2">
                    ...
                    <Dialog open={addDialogOpen} onOpenChange={setAddDialogOpen}>
                        ...
                        <DialogContent className="glass-card border-white/10 sm:max-w-[400px]">
                            <DialogHeader>
                                <DialogTitle className="text-lg">Add Mutual Fund</DialogTitle>
                            </DialogHeader>
                            <form onSubmit={handleAddFund} className="space-y-4 py-2">
                                <div className="space-y-1.5 relative">
                                    <Label className="text-xs">Scheme Name or Symbol</Label>
                                    <Popover
                                        open={searchOpen}
                                    ...
                                    >
                                        <PopoverTrigger asChild>
                                            <div className="relative">
                                                ...
                                                <Input
                                                    placeholder="Search (e.g. Axis Bluechip, HDFC)"
                                        ...
                                        >
                                            <Command className="bg-transparent">
                                                <CommandList className="max-h-[200px]">
                                                    <CommandEmpty className="py-2 text-xs text-center">No funds found.</CommandEmpty>
                                ...
                                <div className="grid grid-cols-2 gap-4">
                                    ...
                                    <div className="space-y-1.5">
                                        <Label className="text-xs">Avg. Nav / Price</Label>
                                        <Input
                                            type="number"
                                            step="any"
                                            placeholder="0.00"
            ...
            {/* Professional Compact MF Table */}
            <GlassCard className="overflow-hidden border-white/5">
                <div className="overflow-x-auto">
                    <table className="w-full text-left border-collapse">
                        <thead>
                            <tr className="border-b border-white/5 bg-white/[0.03]">
                                <th className="px-4 py-2.5 text-[10px] font-black uppercase tracking-widest text-muted-foreground w-1/4">Scheme Name</th>
                                <th className="px-4 py-2.5 text-[10px] font-black uppercase tracking-widest text-muted-foreground w-1/4">Position</th>
                                <th className="px-4 py-2.5 text-[10px] font-black uppercase tracking-widest text-muted-foreground w-1/6">Current NAV</th>
                        ...
                        <tbody className="divide-y divide-white/5">
                            {funds.length > 0 ? funds.map((fund) => {
                                const investment = fund.quantity * fund.avgPrice;
                                const currentVal = fund.quantity * fund.currentPrice;
                                const pl = currentVal - investment;
                                const plPercent = investment > 0 ? (pl / investment) * 100 : 0;

                                return (
                                    <tr key={fund.id} className="hover:bg-white/[0.015] transition-colors group">
                                        <td className="px-4 py-2.5">
                                            <p className="font-bold text-sm tracking-tight text-primary-foreground">{fund.symbol}</p>
                                            <p className="text-[9px] text-muted-foreground font-medium uppercase tracking-tighter">Avg Nav ₹{fund.avgPrice.toLocaleString()}</p>
                                        </td>
                                        <td className="px-4 py-2.5">
                                            <p className="font-bold text-xs">{fund.quantity.toFixed(3)} Units</p>
                                            <p className="text-[10px] text-muted-foreground">₹{investment.toLocaleString()}</p>
                                        </td>
                                        <td className="px-4 py-2.5">
                                            <p className="font-bold text-xs">₹{fund.currentPrice.toLocaleString()}</p>
                                            <div className={cn("flex items-center gap-1 text-[9px] font-black leading-none", fund.change >= 0 ? "text-accent" : "text-destructive")}>
                                                {fund.change >= 0 ? <TrendingUp className="h-2.5 w-2.5" /> : <TrendingDown className="h-2.5 w-2.5" />}
                                                ...
                                                <Button
                                                    ...
                                                    onClick={() => {
                                                        ...
                                                        setFormData({
                                                            ...
                                                            avgPrice: fund.avgPrice.toString()
                                                        });
                                                    }}
                                                >
                                ...
                                <tr>
                                    <td colSpan={5} className="p-16 text-center">
                                        <div className="flex flex-col items-center gap-2 opacity-20">
                                            ...
                                            <p className="text-[10px] font-bold uppercase tracking-widest">MF Portfolio is Empty</p>
                                        </div>
                                    </td>
                                </tr>
                            )}
            ...
            <Dialog open={!!editingFund} onOpenChange={(open) => !open && setEditingFund(null)}>
                <DialogContent className="glass-card border-white/10 sm:max-w-[400px]">
                    <DialogHeader>
                        <DialogTitle className="text-lg">Edit Holdings: {editingFund?.symbol}</DialogTitle>
                    </DialogHeader>
                    <form onSubmit={handleUpdateFund} className="space-y-4 py-2">
                        <div className="grid grid-cols-2 gap-4">
                            <div className="space-y-1.5">
                            ...
                            <div className="space-y-1.5">
                                <Label className="text-xs font-bold uppercase text-muted-foreground">Avg. Nav / Price</Label>
                                <Input
                                    type="number"
                                    step="any"
                                    value={formData.avgPrice}
                        ...
                        <div className="flex gap-2 pt-2">
                            <Button type="button" variant="outline" className="flex-1 bg-white/5 border-white/10" onClick={() => setEditingFund(null)}>
                                Cancel
                            </Button>
